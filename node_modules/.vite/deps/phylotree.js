...
// ../../../../node_modules/phylotree/phylotree.js
var require_phylotree = __commonJS({
  "../../../../node_modules/phylotree/phylotree.js"(exports) {
    (function() {
      var d3_layout_phylotree_event_id = "d3.layout.phylotree.event", d3_layout_phylotree_context_menu_id = "d3_layout_phylotree_context_menu";
      ...
      d3.layout.phylotree = function(container) {
        var item_selected = d3_phylotree_item_selected, node_visible = d3_phylotree_node_visible, node_notshown = d3_phylotree_node_notshown, edge_visible = d3_phylotree_edge_visible, item_tagged = d3_phylotree_item_tagged, resize_svg = d3_phylotree_resize_svg, is_leafnode = d3_phylotree_is_leafnode, has_hidden_nodes = d3_phylotree_has_hidden_nodes, is_node_collapsed = d3_phylotree_is_node_collapsed, node_css_selectors = d3_phylotree_node_css_selectors, edge_css_selectors = d3_phylotree_edge_css_selectors, clade_css_selectors = d3_phylotree_clade_css_selectors, newick_parser = d3_phylotree_newick_parser, rootpath = d3_phylotree_rootpath, rescale = d3_phylotree_rescale, trigger_refresh = d3_phylotree_trigger_refresh, trigger_count_update = d3_phylotree_trigger_count_update, event_listener = d3_phylotree_event_listener, add_event_listener = d3_phylotree_add_event_listener, svg_translate = d3_phylotree_svg_translate, svg_rotate = d3_phylotree_svg_rotate;
        var self = {}, d3_hierarchy = d3.layout.hierarchy().sort(null).value(null), size = [1, 1], phylo_attr = [1, 1], newick_string = null, separation = function(_node, _previos) {
          return 0;
          ...
          return void 0;
        }, branch_length_accessor = def_branch_length_accessor, def_node_label = function(_node) {
          if (options["internal-names"] || d3_phylotree_is_leafnode(_node)) {
            return _node.name || "";
          }
          return "";
        }, node_label = def_node_label, length_attribute = null, scale_attribute = "y_scaled", needs_redraw = true, svg = null, options = {
          ...
          "clade": "clade"
        }, nodes = [], links = [], partitions = [], x_coord = function(d) {
          return d.y;
        }, y_coord = function(d) {
          return d.x;
        }, scales = [1, 1], fixed_width = [15, 20], font_size = 12, scale_bar_font_size = 12, offsets = [0, font_size], draw_line = d3.svg.line().x(function(d) {
          return x_coord(d);
        }).y(function(d) {
          return y_coord(d);
        }).interpolate("step-before"), draw_arc = function(points) {
          var start = radial_mapper(points[0].radius, points[0].angle), end = radial_mapper(points[0].radius, points[1].angle);
          return "M " + x_coord(start) + "," + y_coord(start) + " A " + points[0].radius + "," + points[0].radius + " 0,0, " + (points[1].angle > points[0].angle ? 1 : 0) + " " + x_coord(end) + "," + y_coord(end) + " L " + x_coord(points[1]) + "," + y_coord(points[1]);
        };
        ...
        }, node_styler = void 0, edge_styler = void 0, shown_font_size = font_size, selection_attribute_name = "selected", popover_displayed = null, right_most_leaf = 0, label_width = 0, radial_center = 0, radius = 1, radius_pad_for_bubbles = 0, radial_mapper = function(r2, a2) {
          return {
            "x": radial_center + r2 * Math.sin(a2),
            "y": radial_center + r2 * Math.cos(a2)
          };
        }, cartesian_mapper = function(x, y) {
          return polar_to_cartesian(x - radial_center, y - radial_center);
        }, cartesian_to_polar = function(node, radius2, radial_root_offset) {
          node.x *= scales[0];
          node.y *= scales[1];
          node.radius = radius2 * (node.y / size[1] + radial_root_offset);
          if (!node.angle) {
        ...
        phylotree.placenodes = function() {
          var x = 0, _extents = [
            [0, 0],
            [0, 0]
          ], last_node = null, last_span = 0, save_x = x, save_span = last_span * 0.5;
          var do_scaling = options["scaling"], undef_BL = false, is_under_collapsed_parent = false, max_depth = 1, leaf_counter = 0;
          function process_internal_node(a_node) {
            var count_undefined = 0;
            a_node.x = a_node.children.map(tree_layout).reduce(function(a2, b) {
              if (typeof b == "number") return a2 + b;
              count_undefined += 1;
              return a2;
            }, 0);
          ...
          if (phylotree.radial()) {
            ...
            var last_child_angle = null, last_circ_position = null, last_child_radius = null, min_radius = 0, zero_length = null, effective_span = _extents[0][1] * scales[0];
            var compute_distance = function(r1, r2, a1, a2, annular_shift2) {
              annular_shift2 = annular_shift2 || 0;
              return Math.sqrt((r2 - r1) * (r2 - r1) + 2 * (r1 + annular_shift2) * (r2 + annular_shift2) * (1 - Math.cos(a1 - a2)));
            };
            var max_r = 0;
            ...
            do_lr();
            var annular_shift = 0, do_tip_offset = phylotree.align_tips() && !options["draw-size-bubbles"];
            nodes.forEach(function(d) {
              if (!d.children) {
                var my_circ_position = d.x * scales[0];
                if (last_child_angle !== null) {
                  var required_spacing = my_circ_position - last_circ_position, radial_dist = compute_distance(d.radius, last_child_radius, d.angle, last_child_angle, annular_shift);
                  var local_mr = radial_dist > 0 ? required_spacing / radial_dist : 10 * options["max-radius"];
                  if (local_mr > options["max-radius"]) {
                    var dd = required_spacing / options["max-radius"], b = d.radius + last_child_radius, c = d.radius * last_child_radius - (dd * dd - (last_child_radius - d.radius) * (last_child_radius - d.radius)) / 2 / (1 - Math.cos(last_child_angle - d.angle)), st = Math.sqrt(b * b - 4 * c);
          ...
          if (draw_scale_bar) {
            ...
            var scale = d3.scale.linear().domain([0, domain_limit]).range([shown_font_size, shown_font_size + range_limit]), scaleTickFormatter = d3.format(".2g");
            draw_scale_bar = d3.svg.axis().scale(scale).orient("top").tickFormat(function(d) {
              if (d === 0) {
                return "";
              }
        ...
        phylotree.handle_node_click = function(node) {
          ...
          if (menu_object.empty()) {
            menu_object = d3.select(self.container).append("ul").attr("id", d3_layout_phylotree_context_menu_id).attr("class", "dropdown-menu").attr("role", "menu");
          }
          menu_object.selectAll("li").remove();
          if (node) {
            if (!d3_phylotree_is_leafnode(node)) {
              if (options["collapsible"]) {
                menu_object.append("li").append("a").attr("tabindex", "-1").text(d3_phylotree_is_node_collapsed(node) ? "Expand Subtree" : "Collapse Subtree").on("click", function(d) {
              ...
              if (options["selectable"]) {
                menu_object.append("li").append("a").attr("tabindex", "-1").text("All descendant branches").on("click", function(d) {
                  menu_object.style("display", "none");
                  phylotree.modify_selection(phylotree.select_all_descendants(node, true, true));
                });
                menu_object.append("li").append("a").attr("tabindex", "-1").text("All terminal branches").on("click", function(d) {
                  menu_object.style("display", "none");
                  phylotree.modify_selection(phylotree.select_all_descendants(node, true, false));
                });
                menu_object.append("li").append("a").attr("tabindex", "-1").text("All internal branches").on("click", function(d) {
                  menu_object.style("display", "none");
            ...
            if (node.parent) {
              if (options["selectable"]) {
                menu_object.append("li").append("a").attr("tabindex", "-1").text("Incident branch").on("click", function(d) {
                  menu_object.style("display", "none");
                  phylotree.modify_selection([node]);
                });
                menu_object.append("li").append("a").attr("tabindex", "-1").text("Path to root").on("click", function(d) {
                  menu_object.style("display", "none");
                  phylotree.modify_selection(phylotree.path_to_root(node));
                });
                menu_object.append("li").attr("class", "divider");
              }
              menu_object.append("li").append("a").attr("tabindex", "-1").text("Reroot on this node").on("click", function(d) {
                menu_object.style("display", "none");
                phylotree.reroot(node).update();
              });
              menu_object.append("li").attr("class", "divider");
              menu_object.append("li").append("a").attr("tabindex", "-1").text("Hide this " + (d3_phylotree_is_leafnode(node) ? "node" : "subtree")).on("click", function(d) {
                menu_object.style("display", "none");
                phylotree.modify_selection([node], "notshown", true, true).update_has_hidden_nodes().update();
              });
            }
            if (d3_phylotree_has_hidden_nodes(node)) {
              menu_object.append("li").append("a").attr("tabindex", "-1").text("Show all descendant nodes").on("click", function(d) {
                menu_object.style("display", "none");
            ...
            if (has_user_elements.length) {
              ...
              has_user_elements.forEach(function(d) {
                menu_object.append("li").append("a").attr("tabindex", "-1").text(d[0](node)).on("click", d[1]);
              });
            }
            var tree_container = $(self.container);
            var coordinates = d3.mouse(tree_container[0]);
            menu_object.style("position", "absolute").style("left", "" + coordinates[0] + "px").style("top", "" + coordinates[1] + "px").style("display", "block");
          } else {
            menu_object.style("display", "none");
          }
        };
        ...
        phylotree.update_layout = function(new_json2, do_hierarchy) {
          if (do_hierarchy) {
            nodes = d3_hierarchy.call(this, new_json2);
            nodes.forEach(function(d) {
              d.id = null;
            });
          }
        ...
        phylotree.internal_label = function(callback, respect_existing) {
          ...
          phylotree.modify_selection(function(d2, callback2) {
            if (d3_phylotree_is_leafnode(d2.target)) {
              return d2.target[selection_attribute_name];
            }
            return d2.target[selection_attribute_name];
          });
        };
        ...
        phylotree.resort_children = function(comparator) {
          function sort_children(node) {
            if (node.children) {
              for (var k2 = 0; k2 < node.children.length; k2++) {
                sort_children(node.children[k2]);
              }
        ...
        phylotree.reroot = function(node) {
          if (node.parent) {
            ...
            var remove_me = node, current_node = node.parent, parent_length = current_node.__mapped_bl, stashed_bl = _.noop();
            if (current_node.parent) {
              node.__mapped_bl = node.__mapped_bl === void 0 ? void 0 : node.__mapped_bl * 0.5;
        ...
        phylotree.update = function(transitions) {
          ...
          if (draw_scale_bar) {
            ...
            scale_bar.attr("class", css_classes["tree-scale-bar"]).style("font-size", "" + scale_bar_font_size).attr("transform", function(d) {
          ...
          var drawn_links = enclosure.selectAll(d3_phylotree_edge_css_selectors(css_classes)).data(links.filter(d3_phylotree_edge_visible), function(d) {
          ...
          var collapsed_clades = enclosure.selectAll(d3_phylotree_clade_css_selectors(css_classes)).data(nodes.filter(d3_phylotree_is_node_collapsed), function(d) {
          ...
          if (phylotree.radial()) {
            ...
            spline = d3.svg.line().interpolate(interpolator).y(function(d) {
              return d[0];
            }).x(function(d) {
              return d[1];
            });
            ...
            spline = d3.svg.line().interpolate("basis").y(function(d) {
              return d[0];
            }).x(function(d) {
              return d[1];
            });
          ...
          var drawn_nodes = enclosure.selectAll(d3_phylotree_node_css_selectors(css_classes)).data(nodes.filter(d3_phylotree_node_visible), function(d) {
            return d.id || (d.id = ++node_id);
          });
          ...
          brush.enter().insert("g", ":first-child").attr("class", css_classes["tree-selection-brush"]);
          var brush_object = d3.svg.brush().x(d3.scale.identity().domain([0, sizes[0] - offsets[1] - options["left-offset"]])).y(d3.scale.identity().domain([0, sizes[1] - phylotree.pad_height()])).on("brush", function() {
            var extent = d3.event.target.extent(), shown_links = links.filter(d3_phylotree_edge_visible), selected_links = shown_links.filter(function(d, i) {
        ...
        phylotree.refresh = function() {
          ...
          var nodes2 = enclosure.selectAll(d3_phylotree_node_css_selectors(css_classes));
          nodes2.attr("class", phylotree.reclass_node);
          if (node_styler) {
        ...
        phylotree.draw_edge = function(container2, edge, transition) {
          container2 = d3.select(container2);
          container2.attr("class", phylotree.reclass_edge).on("click", function(d) {
            phylotree.modify_selection([d.target], selection_attribute_name);
          });
          var new_branch_path = draw_branch([edge.source, edge.target]);
          if (transition) {
            if (container2.datum().existing_path) {
              container2.attr("d", function(d) {
                return d.existing_path;
              });
            }
            container2.transition().attr("d", new_branch_path);
          } else {
            container2.attr("d", new_branch_path);
          }
          edge.existing_path = new_branch_path;
          var bl = branch_length_accessor(edge.target);
          if (bl !== void 0) {
            var haz_title = container2.selectAll("title");
            if (haz_title.empty()) {
              haz_title = container2.append("title");
            }
            haz_title.text("Length = " + bl);
          } else {
            container2.selectAll("title").remove();
          }
          if (edge_styler) {
            edge_styler(container2, edge);
          }
          return phylotree;
        };
        ...
        phylotree.draw_node = function(container2, node, transitions) {
          container2 = d3.select(container2);
          if (d3_phylotree_is_leafnode(node)) {
            var labels = container2.selectAll("text").data([node]), tracers = container2.selectAll("line");
            if (transitions) {
            ...
            if (phylotree.radial()) {
              ...
              (transitions ? labels.transition() : labels).attr("text-anchor", "start").attr("transform", function(d) {
            ...
            if (options["draw-size-bubbles"]) {
              ...
              var circles = container2.selectAll("circle").data([shift]);
            ...
            var circles = container2.selectAll("circle").data([node]), radius2 = phylotree.node_circle_size()(node);
            if (radius2 > 0) {
              circles.enter().append("circle");
              circles.attr("r", function(d) {
                return Math.min(shown_font_size * 0.75, radius2);
              }).on("click", function(d) {
                phylotree.handle_node_click(d);
              });
            } else {
        ...
        phylotree.set_partitions = function(partitions2) {
      ...
      function d3_phylotree_resize_svg(tree, svg, tr) {
        var sizes = tree.size();
        if (tree.radial()) {
          var pad_radius = tree.pad_width(), vertical_offset = tree.options()["top-bottom-spacing"] != "fit-to-size" ? tree.pad_height() : 0;
          sizes = [
      ...
      function d3_phylotree_node_css_selectors(css_classes) {
        return [css_classes["node"], css_classes["internal-node"], css_classes["collapsed-node"], css_classes["tagged-node"]].reduce(function(p, c, i, a2) {
          return p += "g." + c + (i < a2.length - 1 ? "," : "");
        }, "");
      }
      function d3_phylotree_edge_css_selectors(css_classes) {
        return [css_classes["branch"], css_classes["selected-branch"], css_classes["tagged-branch"]].reduce(function(p, c, i, a2) {
          return p += "path." + c + (i < a2.length - 1 ? "," : "");
        }, "");
      }
      function d3_phylotree_clade_css_selectors(css_classes) {
        return [css_classes["clade"]].reduce(function(p, c, i, a2) {
          return p += "path." + c + (i < a2.length - 1 ? "," : "");
        }, "");
      }
      ...
      function d3_phylotree_svg_rotate(a2) {
        if (a2 !== null) {
          return "rotate (" + a2 + ") ";
        }
        return "";
...
export default require_phylotree();
